...
export class OrderService {
  ...
  protected convertDateFromClient(order: IOrder): IOrder {
    const copy: IOrder = Object.assign({}, order, {
      dateAdded: order.dateAdded && order.dateAdded.isValid() ? order.dateAdded.toJSON() : undefined,
      dateModified: order.dateModified && order.dateModified.isValid() ? order.dateModified.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((order: IOrder) => {
        order.dateAdded = order.dateAdded ? moment(order.dateAdded) : undefined;
        order.dateModified = order.dateModified ? moment(order.dateModified) : undefined;
      });
    }
    return res;
  }
